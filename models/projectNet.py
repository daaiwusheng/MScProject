...
class ProjectStatisticsNeuralNet(ProjectNeuralNetAbstract):
    """
    Attention Neural Net, with more detailed statistics method
    Args:
        -patchproject (str): path project
        -nameproject (str):  name project
        -no_cuda (bool): system cuda (default is True)
    ...
    def create(self,
        ...
        Args:
            ...
            -
        """
        super(ProjectStatisticsNeuralNet, self).create(
            arch,
            num_output_channels,
            num_input_channels,
            loss,
    ...
    def evaluate(self, data_loader, epoch=0):
        ...
        with torch.no_grad():
            ...
            for i, (x_img, label) in enumerate(data_loader):
                ...
                acc = float(correct) / label.size(0)
                # update
                self.logger_val.update(
                    {'loss': loss.cpu().item()},
                    {'accuracy': acc},
        ...
        self.vallosses = self.logger_val.info['loss']['loss'].avg
        acc_average = self.logger_val.info['metrics']['accuracy'].avg
